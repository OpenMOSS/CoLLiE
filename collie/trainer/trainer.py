...
class Trainer:
    ...
    def setup_parallel_model(self):
        ...
        self.engine, self.optimizer, self.train_dataloader, _ = deepspeed.initialize(
            model=self.model,
    ...
    def train(self, dataloader: Optional[Iterable] = None):
        ...
        with tqdm.tqdm(range(self.args.train_epochs), desc="Training Epoch: ", disable=dist.get_rank() != 0) as tqbar_epoch:
            for epoch_idx in tqbar_epoch:
                with tqdm.tqdm(train_dataloader, desc="Training Batch: ", disable=dist.get_rank() != 0) as tqbar_batch:
    ...
    def eval(self, 
        ...
        with tqdm.tqdm(eval_dataloader, desc="Evaluating Batch: ", disable=dist.get_rank() != 0) as tqbar_batch:
            for batch_idx, batch in enumerate(tqbar_batch):
                ...
                tqbar_batch.set_postfix(
                            batch=f"{batch_idx + 1}/{len(self.eval_dataset) / self.args.eval_batch_size}")
