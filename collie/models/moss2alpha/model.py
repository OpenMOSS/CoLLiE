...
from sparse_varlen_kernel import flash_attn_varlen_func as sparse_varlen_func
...
class MOSS2SparseAttention2(MOSS2FlashAttention2):
    """
    MOSS2 sparse flash attention module. Only return attention output. Only support causal attention.
    Attention bias, dropout and determistic backward are not supported.
    This module inherits from `MOSS2FlashAttention2` as the weights of the module stays
    untouched. The only required change would be on the forward pass where it needs to correctly call the public API of
    flash attention and deal with padding tokens in case the input contains any of them.
    """
    ...
    def forward(
        ...
        **kwargs,
        ...
        # MOSS2SparseAttention2 attention does not support output_attentions
        if "padding_mask" in kwargs:
            warnings.warn(
                "Passing `padding_mask` is deprecated and will be removed in v4.37. "
                "Please make sure use `attention_mask` instead.`"
    ...
    def _flash_attention_forward(
        ...
        Args:
            ...
            attention_mask (`torch.Tensor`):
                ...
                position of padding tokens and 1 for the position of non-padding tokens.
            dropout (`int`, *optional*):
                Attention dropout, not supported in sparse attention
            softmax_scale (`float`, *optional*):
                The scaling of QK^T before applying softmax. Default to 1 / sqrt(head_dim)
        """
        # Contains at least one padding token in the sequence
        ...
        if attention_mask is not None:
            ...
            attn_output_unpad = sparse_varlen_func(
                query_states,
                key_states,
                value_states,
                cu_seqlens_q,
...
class MOSS2DecoderLayer(nn.Module):
    def __init__(self, config: CollieConfig, layer_idx):
        ...
        full_attn_list: Optional[List] = config.model_config.full_attention_layers
        # self.attention = MOSS2_ATTENTION_CLASSES[config.attn_implementation](config=config)
        if full_attn_list is not None \
            and layer_idx not in full_attn_list:
                self.attention = MOSS2SparseAttention2(config=config)
        else:
            if config.attn_implementation == "flash_attention_2" or config.use_flash:
                self.attention = MOSS2FlashAttention2(config=config)
            else:
                self.attention = MOSS2Attention(config=config)  
...
class MOSS2Model(nn.Module):
    ...
    def __init__(self, config: CollieConfig):
        ...
        self.layers = nn.ModuleList([MOSS2DecoderLayer(config, i) for i in range(config.num_hidden_layers)])
    ...
    def pipeline_layers(cls, config: CollieConfig):
        ...
        layers = [
            LayerSpec(MOSS2DecoderLayer, config, i) for i in range(config.num_hidden_layers)
        ]
        norm = LayerSpec(
...
class MOSS2ForCausalLM(CollieModelForCausalLM):
    ...
    def chat(
        ...
        top_p: float = 0.8,
        meta_instruction: str = "You are an AI assistant whose name is MOSS2. You can chat with me.",
        **kwargs,
    ):
    ...
    def pipeline_layers(cls, config: CollieConfig):
        ...
        return [("model", MOSS2Model.pipeline_layers(config)), ("output", output)]
