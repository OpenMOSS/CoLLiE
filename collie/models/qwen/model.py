...
class QWenAttention(nn.Module):
    def __init__(self, config):
        ...
        self.c_attn = ColumnParallelLinearWithoutBias(
            config.hidden_size,
        ...
        # )
        self.c_proj = RowParallelLinearWithoutBias(
            config.hidden_size,
    ...
    def forward(
        ...
        tp_size = 3 * self.projection_size // int(mixed_x_layer.shape[2])
        
        query, key, value = mixed_x_layer.split(self.split_size // tp_size, dim=2)
        
        query = self._split_heads(query, self.num_heads // tp_size, self.head_dim)
        key = self._split_heads(key, self.num_heads // tp_size, self.head_dim)
        value = self._split_heads(value, self.num_heads // tp_size, self.head_dim)
        ...
        context_layer = self._merge_heads(
            attn_output, self.num_heads // tp_size, self.head_dim
        )
...
class QWenMLP(nn.Module):
    def __init__(self, config):
        ...
        self.w2 = ColumnParallelLinearWithoutBias(
            config.hidden_size,
            config.intermediate_size // 2,
            bias=not config.no_bias,
            gather_output=False,
...
class QWenModel(nn.Module):
    ...
    def __init__(self, config: CollieConfig):
        ...
        self.wte = tensor_parallel.VocabParallelEmbedding(
            self.vocab_size, self.embed_dim, params_dtype=torch.float32
        )
...
class QWenLMHeadModel(CollieModelForCausalLM):
    ...
    def load_parallel_state_dict(
        ...
        for rank in rank_order:
            ...
            if int(os.environ.get("RANK", "0")) == rank or not process_exclusion:
                ...
                if (
                        io_driver.exists(os.path.join(path, "model.safetensors.index.json"))
                        and "COLLIE_PP_PARTS" in os.environ.keys()
                ):
                    weight_map = json.loads(
                        io_driver.load(
                            os.path.join(path, "model.safetensors.index.json"), mode="r"
                        )
                    )["weight_map"]
                    # layers 表示自己需要的层
                    ...
                    if 0 in layers:
                        weights.append(weight_map["transformer.wte.weight"])
                    if max(parts) - 1 in layers:
                        weights.append(weight_map["lm_head.weight"])
                    if max(parts) - 2 in layers:
                        weights.append(weight_map["transformer.ln_f.weight"])
                else:
                ...
                for key in list(state_dict.keys()):
                    ...
                    elif key.endswith("c_attn.weight") or key.endswith("c_attn.bias"):
                        weights = torch.chunk(state_dict[key], 3, dim=0)
                        chunks = []
                        for weight in weights:
                            tensor = (
                                list(torch.chunk(weight, config.tp_size, dim=0))[
                                    env.tp_rank
                                ]
                                .detach()
                                .clone()
                    ...
                    elif key.endswith("c_attn.bias"):
                        pass
                    elif key.endswith("c_proj.weight"):
                        tensor = (
                        ...
                        if env.tp_rank == 0:
                            ...
                            if col_split:
                                ...
                                for tensor in tensor_list:
                                    weights = torch.chunk(tensor, 3, dim=0)
                                    weights_q.append(weights[0])
                                    weights_k.append(weights[1])
                                    weights_v.append(weights[2])
                                tensor_list = weights_q + weights_k + weights_v
                                state_dict[key] = concat_tensor(tensor_list, dim=0)
