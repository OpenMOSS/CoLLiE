...
class InternLMModel(nn.Module):
    ...
    def pipeline_layers(cls, config: CollieConfig):
        ...
        layers = [
            LayerSpec(InternLMLayer, config, i) for i in range(config.num_hidden_layers)
        ]
        norm = LayerSpec(
...
class InternLMForCausalLM(CollieModelForCausalLM):
    ...
    def pipeline_layers(cls, config: CollieConfig):
        ...
        return [("model", InternLMModel.pipeline_layers(config)), ("lm_head", lm_head)]
                ...
                with progress(
                    ...
                    for weight in pbar:
                        ...
                        for key in list(part_state_dict.keys()):
                            ...
                            if key.endswith("q_proj.weight") or key.endswith(
                                "k_proj.weight"
                            ):
                                part_state_dict[key] = (
                                    rearrange(
                            ...
                            if key.endswith("q_proj.bias") or key.endswith(
                                "k_proj.bias"
                            ):
                                part_state_dict[key] = (
                                    rearrange(
