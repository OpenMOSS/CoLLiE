...
sys.path.append("../..")
from collie.metrics.classify_f1_pre_rec_metric import ClassifyFPreRecMetric


class TestClassifyFPreRecMetric:
    ...
    def test_case_1(self):
        ...
        target = torch.tensor([0, 2, 4, 1, 4, 0, 1, 3, 3, 3, 1, 3, 4, 4, 3, 4, 0, 2, 4, 4, 3, 4, 4, 3,
                               0, 3, 0, 0, 0, 1, 3, 1])
        input_dict = {'pred': pred, 'target': target}
    ...
    def test_case_2(self):
        ...
        target = torch.tensor([0, 2, 4, 1, 4, 0, 1, 3, 3, 3, 1, 3, 4, 4, 3, 4, 0, 2, 4, 4, 3, 4, 4, 3,
                               0, 3, 0, 0, 0, 1, 3, 1])
        pred = list(torch.chunk(pred, 4, dim=0))
        target = list(torch.chunk(target, 4, dim=0))
        input_dict = {'pred': pred, 'target': target}
        metric = ClassifyFPreRecMetric(f_type='macro')
        metric.update(input_dict)
        result_dict = metric.get_metric()
